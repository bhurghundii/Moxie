...
function addFriendManually() {
  ...
  if (name && id) {
    ...
    var buddyListFile = ""
    if (isReleaseBuild() == 0) {
      buddyListFile = 'torchat/buddy-list.txt'
    }
    if (isReleaseBuild() == 1) {
      buddyListFile = 'torchat/dist/torchat/buddy-list.txt'
    }
    if (isReleaseBuild() == 2) {
      buddyListFile = 'torchat/dist/buddy-list.txt'
    }

    fs.appendFile(buddyListFile, id + ' ' + name + '\n', function(err) {
      if (err) throw err;
